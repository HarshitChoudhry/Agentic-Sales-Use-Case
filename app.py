...
upcoming_choice = None
past_choice = None

if os.path.exists(meetings_file):
    df_all = pd.read_excel(meetings_file, None)  # dict of all sheets
    
    upcoming_options, past_options = [], []
    for client, df in df_all.items():
        if "Status" in df.columns:
            for _, row in df[df["Status"].str.lower() == "upcoming"].iterrows():
                upcoming_options.append(f"{row['Meeting_ID']} | {client} | {row['Title']}")
            for _, row in df[df["Status"].str.lower() == "past"].iterrows():
                past_options.append(f"{row['Meeting_ID']} | {client} | {row['Title']}")

    # Dropdowns
    upcoming_choice = st.selectbox("📅 Select an Upcoming Meeting (for Prep Pack):", ["-- Select --"] + upcoming_options)
    past_choice = st.selectbox("📜 Select a Past Meeting (for Coaching / Deal Execution):", ["-- Select --"] + past_options)

# --- Upcoming Meeting Actions ---
if upcoming_choice and upcoming_choice != "-- Select --":
    meeting_id, client_name, title = [x.strip() for x in upcoming_choice.split("|")]
    ...
    with col2:
        if st.button("📄 Generate Account Summary"):
            agent = DealExecutionAgent(crm_path=crm_file, meetings_path=meetings_file)
            result = agent.process_account(client_name)
            st.success(f"Account Summary generated for {client_name}")
            summary_file = f"outputs/deal_summaries/{client_name}_account_summary.pdf"
            if os.path.exists(summary_file):
                with open(summary_file, "rb") as f:
                    st.download_button("⬇️ Download Account Summary", f, file_name=os.path.basename(summary_file))

# --- Past Meeting Actions ---
if past_choice and past_choice != "-- Select --":
    meeting_id, client_name, title = [x.strip() for x in past_choice.split("|")]
    ...
    with col2:
        if st.button("📄 Generate Deal Execution Summary"):
            agent = DealExecutionAgent(crm_path=crm_file, meetings_path=meetings_file)
            result = agent.process(meeting_id, client_name)
            st.success(f"Deal Execution Summary generated for {client_name} - {meeting_id}")
            summary_file = f"outputs/deal_summaries/{client_name}_{meeting_id}_summary.pdf"
            if os.path.exists(summary_file):
                with open(summary_file, "rb") as f:
                    st.download_button("⬇️ Download Deal Summary", f, file_name=os.path.basename(summary_file))
